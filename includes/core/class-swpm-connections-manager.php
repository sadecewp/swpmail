...
/**
 * Manages provider connections and credentials.
 */
class SWPM_Connections_Manager implements SWPM_Provider_Interface {
	...
	/**
	 * Backup provider (null if not configured).
	 *
	 * @var SWPM_Provider_Interface|null
	 */
...
// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	...
	private function reset_failures( string $slot ): void {
		...
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery, WordPress.DB.PreparedSQL.InterpolatedNotPrepared		$wpdb->query( "SELECT GET_LOCK('swpm_health_{$slot}', 2)" );
