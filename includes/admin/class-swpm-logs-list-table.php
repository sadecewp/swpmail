...
class SWPM_Logs_List_Table extends WP_List_Table {
	...
	public function column_provider_used( $item ): string {
		return esc_html( $item->provider_used ? $item->provider_used : '—' );
	}
	...
	public function column_sent_at( $item ): string {
		$date = $item->sent_at ? $item->sent_at : $item->created_at;
		if ( empty( $date ) ) {
			return '—';
		}
		$ts = strtotime( $date );
...
// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.MissingUnslash
	...
	public static function ajax_tracking_detail(): void {
		...
		$events = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT event_type, url, ip_address, user_agent, created_at FROM {$tracking_table} WHERE queue_id = %d ORDER BY created_at DESC",
				$queue_id
			),
			ARRAY_A
		);
