...
class SWPM_Subscriber {
	...
	public function create( string $email, string $name = '', string $frequency = 'instant' ) {
		...
		$blocked = apply_filters( 'swpm_blocked_email_domains', array(
			'mailinator.com',
			'tempmail.com',
			'guerrillamail.com',
			'10minutemail.com',
	...
	public function get_confirmed_by_frequency( string $frequency, int $limit = 100, int $offset = 0 ): array {
		...
		return $this->db->get_results(
			$this->db->prepare(
				"SELECT * FROM {$this->table} WHERE status = 'confirmed' AND frequency = %s ORDER BY id ASC LIMIT %d OFFSET %d", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				$frequency,
				$limit,
				$offset
			)
	...
	public function export_personal_data( string $email_address, int $page = 1 ): array {
		...
		if ( $subscriber ) {
			$data[] = array(
				...
				'data'        => array(
					array( 'name' => __( 'Email', 'swpmail' ), 'value' => $subscriber->email ),
					array( 'name' => __( 'Name', 'swpmail' ), 'value' => $subscriber->name ),
					array( 'name' => __( 'Status', 'swpmail' ), 'value' => $subscriber->status ),
					array( 'name' => __( 'Frequency', 'swpmail' ), 'value' => $subscriber->frequency ),
					array( 'name' => __( 'IP Address', 'swpmail' ), 'value' => $subscriber->ip_address ),
					array( 'name' => __( 'Subscribed On', 'swpmail' ), 'value' => $subscriber->created_at ),
				),
			);
		}
