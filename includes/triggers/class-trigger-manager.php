...
class SWPM_Trigger_Manager {
	...
	/** @var array<string> Built-in trigger keys. */
	...
	public static function delete_custom_trigger( string $key ): void {
		$triggers = self::get_custom_triggers();
		$triggers = array_values( array_filter( $triggers, function ( $t ) use ( $key ) {
			return ( $t['key'] ?? '' ) !== $key;
		} ) );
		update_option( 'swpm_custom_triggers', $triggers );
		...
		$active = array_values( array_filter( $active, function ( $k ) use ( $key ) {
			return $k !== $key;
		} ) );
