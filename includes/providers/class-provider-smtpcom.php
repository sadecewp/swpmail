...
class SWPM_Provider_SMTPcom implements SWPM_Provider_Interface {
	...
	public function send(
		...
		$payload = array(
			...
			'originator' => array(
				'from' => array( 'name' => $from_name, 'address' => $from_email ),
			),
			'subject' => $subject,
			'body'    => array(
				'parts' => array(
					array( 'type' => 'text/plain', 'content' => wp_strip_all_tags( $body ) ),
					array( 'type' => 'text/html', 'content' => $body ),
				),
			),
		);
		...
		$response = wp_remote_post( 'https://api.smtp.com/v4/messages', array(
