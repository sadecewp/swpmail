...
class SWPM_Queue {
	...
	public function process( int $batch_size = 50 ): void {
		...
		foreach ( $items as $item ) {
			...
			if ( ! is_email( $item->to_email ) ) {
				...
				$this->db->update(
					$this->table,
					array( 'status' => 'failed', 'error_message' => 'Invalid email address.', 'body' => '' ),
					array( 'id' => $item->id ),
					array( '%s', '%s', '%s' ),
					array( '%d' )
				);
			...
			$this->db->update(
				$this->table,
				array( 'status' => 'sending', 'attempts' => $item->attempts + 1 ),
				array( 'id' => $item->id ),
				array( '%s', '%d' ),
				array( '%d' )
			);
			...
			$headers = (array) apply_filters( 'swpm_mail_headers', array_merge(
	...
	public function reschedule_with_delay( int $queue_id, int $delay ): void {
		...
		$this->db->update(
			$this->table,
			array( 'scheduled_at' => $new_time, 'status' => 'pending' ),
			array( 'id' => $queue_id ),
			array( '%s', '%s' ),
			array( '%d' )
		);
