...
class SWPM_DB_Repair {
	...
	private const SCHEMA = array(
		'swpm_subscribers' => array(
			'id', 'email', 'name', 'status', 'frequency', 'token',
			'ip_address', 'confirmed_at', 'created_at', 'updated_at',
		),
		'swpm_queue'       => array(
			'id', 'subscriber_id', 'template_id', 'to_email', 'subject', 'body',
			'headers', 'attachments', 'status', 'attempts', 'max_attempts',
			'provider_used', 'provider_msg_id', 'scheduled_at', 'sent_at',
			'error_message', 'error_code', 'created_at',
		),
		'swpm_logs'        => array(
			'id', 'queue_id', 'trigger_key', 'provider', 'level',
			'message', 'context', 'created_at',
		),
		'swpm_tracking'    => array(
			'id', 'hash', 'queue_id', 'to_email', 'subject', 'event_type',
	...
	public function repair(): array {
		...
		swpm_log( 'info', 'DB repair completed.', array(
			'fixed'  => count( $fixed ),
			'errors' => count( $errors ),
		) );
