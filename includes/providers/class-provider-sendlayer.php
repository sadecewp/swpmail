...
class SWPM_Provider_SendLayer implements SWPM_Provider_Interface {
	...
	public function send(
		...
		$api_key    = swpm_decrypt( get_option( 'swpm_sendlayer_api_key_enc', '' ) );
		...
		if ( empty( $api_key ) ) {
			return SWPM_Send_Result::failure(
				__( 'SendLayer API key is not configured.', 'swpmail' ),
				'MISSING_CONFIG'
			);
		}
		...
		$response = wp_remote_post( 'https://console.sendlayer.com/api/v1/email', array(
		...
		if ( $status >= 200 && $status < 300 ) {
			$msg_id = $parsed['MessageId'] ?? $parsed['Id'] ?? '';
			return SWPM_Send_Result::success( $msg_id, $parsed ?: array() );
		}

		$error_message = $parsed['Message'] ?? $parsed['message'] ?? wp_remote_retrieve_body( $response );

		return SWPM_Send_Result::failure(
			"SendLayer API Error {$status}: {$error_message}",
			"HTTP_{$status}",
			$parsed ?: array()
		);
	}
	...
	public function test_connection(): SWPM_Send_Result {
		return $this->send(
			...
			__( 'SWPMail SendLayer Test', 'swpmail' ),
			__( 'SendLayer connection is working correctly.', 'swpmail' )
		);
	}
