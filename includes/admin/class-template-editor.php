	...
	public function ajax_create_template(): void {
		...
		wp_send_json_success( array(
			...
			'redirect'    => add_query_arg(
				array( 'page' => 'swpmail-templates', 'template' => $id ),
				admin_url( 'admin.php' )
			),
		) );
	}
  ...
  <body>
	...
	public function ajax_create_trigger(): void {
		...
		$core_blocked_hooks = array(
			'delete_post', 'wp_delete_post', 'before_delete_post',
			'delete_user', 'deleted_user', 'remove_user_from_blog',
			'wp_logout', 'clear_auth_cookie',
			'switch_theme', 'activate_plugin', 'deactivate_plugin',
			'wp_insert_post', 'save_post', 'edit_post',
			'update_option', 'delete_option', 'add_option',
			'set_user_role', 'grant_super_admin', 'revoke_super_admin',
			'wp_update_user', 'profile_update',
			'shutdown', 'wp_die_handler',
		);
