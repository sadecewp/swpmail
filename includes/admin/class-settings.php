...
class SWPM_Settings {
	...
	private function register_mail_settings(): void {
		register_setting( 'swpm_mail_settings_group', 'swpm_mail_provider', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_provider' ),
			'default'           => 'phpmail',
		) );

		register_setting( 'swpm_mail_settings_group', 'swpm_from_name', array(
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_text_field',
			'default'           => get_bloginfo( 'name' ),
		) );

		register_setting( 'swpm_mail_settings_group', 'swpm_from_email', array(
			'type'              => 'string',
		...
		// SMTP settings.
		register_setting( 'swpm_mail_settings_group', 'swpm_smtp_host', array(
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_text_field',
		) );
		register_setting( 'swpm_mail_settings_group', 'swpm_smtp_port', array(
			'type'              => 'integer',
			'sanitize_callback' => 'absint',
			'default'           => 587,
		) );
		register_setting( 'swpm_mail_settings_group', 'swpm_smtp_encryption', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_encryption' ),
			'default'           => 'tls',
		) );
		register_setting( 'swpm_mail_settings_group', 'swpm_smtp_username', array(
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_text_field',
		) );
		register_setting( 'swpm_mail_settings_group', 'swpm_smtp_password_enc', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_encrypted_field' ),
		) );

		// Mailgun settings.
		register_setting( 'swpm_mail_settings_group', 'swpm_mailgun_api_key_enc', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_encrypted_field' ),
		) );
		register_setting( 'swpm_mail_settings_group', 'swpm_mailgun_domain', array(
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_text_field',
		) );
		register_setting( 'swpm_mail_settings_group', 'swpm_mailgun_region', array(
			'type'              => 'string',
		...
		// SendGrid settings.
		register_setting( 'swpm_mail_settings_group', 'swpm_sendgrid_api_key_enc', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_encrypted_field' ),
		) );

		// Postmark settings.
		register_setting( 'swpm_mail_settings_group', 'swpm_postmark_server_token_enc', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_encrypted_field' ),
		) );
		register_setting( 'swpm_mail_settings_group', 'swpm_postmark_message_stream', array(
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_text_field',
			'default'           => 'outbound',
		) );
		...
		// Brevo settings.
		register_setting( 'swpm_mail_settings_group', 'swpm_brevo_api_key_enc', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_encrypted_field' ),
		) );

		// Amazon SES settings.
		register_setting( 'swpm_mail_settings_group', 'swpm_ses_access_key_enc', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_encrypted_field' ),
		) );
		register_setting( 'swpm_mail_settings_group', 'swpm_ses_secret_key_enc', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_encrypted_field' ),
		) );
		register_setting( 'swpm_mail_settings_group', 'swpm_ses_region', array(
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_text_field',
			'default'           => 'us-east-1',
		) );
		...
		// Resend settings.
		register_setting( 'swpm_mail_settings_group', 'swpm_resend_api_key_enc', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_encrypted_field' ),
		) );

		// SendLayer settings.
		register_setting( 'swpm_mail_settings_group', 'swpm_sendlayer_api_key_enc', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_encrypted_field' ),
		) );

		// SMTP.com settings.
		register_setting( 'swpm_mail_settings_group', 'swpm_smtpcom_api_key_enc', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_encrypted_field' ),
		) );
		register_setting( 'swpm_mail_settings_group', 'swpm_smtpcom_channel', array(
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_text_field',
		) );

		// Gmail settings.
		register_setting( 'swpm_mail_settings_group', 'swpm_gmail_username', array(
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_email',
		) );
		register_setting( 'swpm_mail_settings_group', 'swpm_gmail_app_password_enc', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_encrypted_field' ),
		) );
		register_setting( 'swpm_mail_settings_group', 'swpm_gmail_oauth_client_id', array(
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_text_field',
		) );
		register_setting( 'swpm_mail_settings_group', 'swpm_gmail_oauth_client_secret_enc', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_encrypted_field' ),
		) );

		// Outlook / 365 settings.
		register_setting( 'swpm_mail_settings_group', 'swpm_outlook_username', array(
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_email',
		) );
		register_setting( 'swpm_mail_settings_group', 'swpm_outlook_password_enc', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_encrypted_field' ),
		) );
		register_setting( 'swpm_mail_settings_group', 'swpm_outlook_oauth_client_id', array(
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_text_field',
		) );
		register_setting( 'swpm_mail_settings_group', 'swpm_outlook_oauth_client_secret_enc', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_encrypted_field' ),
		) );

		// Elastic Email settings.
		register_setting( 'swpm_mail_settings_group', 'swpm_elasticemail_api_key_enc', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_encrypted_field' ),
		) );

		// Mailjet settings.
		register_setting( 'swpm_mail_settings_group', 'swpm_mailjet_api_key_enc', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_encrypted_field' ),
		) );
		register_setting( 'swpm_mail_settings_group', 'swpm_mailjet_secret_key_enc', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_encrypted_field' ),
		) );

		// MailerSend settings.
		register_setting( 'swpm_mail_settings_group', 'swpm_mailersend_api_token_enc', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_encrypted_field' ),
		) );

		// SMTP2GO settings.
		register_setting( 'swpm_mail_settings_group', 'swpm_smtp2go_api_key_enc', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_encrypted_field' ),
		) );

		// SparkPost settings.
		register_setting( 'swpm_mail_settings_group', 'swpm_sparkpost_api_key_enc', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_encrypted_field' ),
		) );
		register_setting( 'swpm_mail_settings_group', 'swpm_sparkpost_region', array(
			'type'              => 'string',
		...
		// Zoho Mail settings.
		register_setting( 'swpm_mail_settings_group', 'swpm_zoho_username', array(
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_email',
		) );
		register_setting( 'swpm_mail_settings_group', 'swpm_zoho_password_enc', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_encrypted_field' ),
		) );
		register_setting( 'swpm_mail_settings_group', 'swpm_zoho_region', array(
			'type'              => 'string',
	...
	private function register_failover_settings(): void {
		register_setting( 'swpm_failover_settings_group', 'swpm_backup_provider', array(
			'type'              => 'string',
	...
	private function register_general_settings(): void {
		register_setting( 'swpm_general_settings_group', 'swpm_double_opt_in', array(
			'type'              => 'boolean',
			'sanitize_callback' => 'rest_sanitize_boolean',
			'default'           => true,
		) );
		register_setting( 'swpm_general_settings_group', 'swpm_gdpr_checkbox', array(
			'type'              => 'boolean',
			'sanitize_callback' => 'rest_sanitize_boolean',
			'default'           => true,
		) );
		register_setting( 'swpm_general_settings_group', 'swpm_gdpr_privacy_page', array(
			'type'              => 'integer',
			'sanitize_callback' => 'absint',
			'default'           => 0,
		) );
		register_setting( 'swpm_general_settings_group', 'swpm_show_frequency_choice', array(
			'type'              => 'boolean',
			'sanitize_callback' => 'rest_sanitize_boolean',
			'default'           => true,
		) );
		register_setting( 'swpm_general_settings_group', 'swpm_form_title', array(
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_text_field',
			'default'           => __( 'Subscribe to Newsletter', 'swpmail' ),
		) );
		register_setting( 'swpm_general_settings_group', 'swpm_notify_admin_on_failure', array(
			'type'              => 'boolean',
			'sanitize_callback' => 'rest_sanitize_boolean',
			'default'           => true,
		) );
		register_setting( 'swpm_general_settings_group', 'swpm_daily_send_hour', array(
			'type'              => 'integer',
			'sanitize_callback' => 'absint',
			'default'           => 9,
		) );
		register_setting( 'swpm_general_settings_group', 'swpm_weekly_send_day', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_weekday' ),
			'default'           => 'monday',
		) );
		register_setting( 'swpm_general_settings_group', 'swpm_override_wp_mail', array(
			'type'              => 'boolean',
			'sanitize_callback' => 'rest_sanitize_boolean',
			'default'           => true,
		) );
		register_setting( 'swpm_general_settings_group', 'swpm_active_triggers', array(
			'type'              => 'array',
		...
		register_setting( 'swpm_general_settings_group', 'swpm_enable_open_tracking', array(
			'type'              => 'boolean',
			'sanitize_callback' => 'rest_sanitize_boolean',
			'default'           => true,
		) );
		register_setting( 'swpm_general_settings_group', 'swpm_enable_click_tracking', array(
			'type'              => 'boolean',
			'sanitize_callback' => 'rest_sanitize_boolean',
			'default'           => true,
		) );
		...
		register_setting( 'swpm_general_settings_group', 'swpm_delete_data_on_uninstall', array(
			'type'              => 'boolean',
			'sanitize_callback' => 'rest_sanitize_boolean',
			'default'           => false,
		) );
