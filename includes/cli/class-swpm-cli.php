...
/**
 * WP-CLI commands for SWPMail.
 // phpcs:ignore Squiz.Commenting.BlockComment.HasEmptyLineBefore
 */
class SWPM_CLI extends WP_CLI_Command {

	// phpcs:ignore Squiz.Commenting.BlockComment.HasEmptyLineBefore, Squiz.Commenting.BlockComment.NoEmptyLineBefore
	// ==================================================================
	// wp swpmail status
	// ================================================================
	...
	public function test( $args, $assoc_args ) {
		...
		$body    = \WP_CLI\Utils\get_flag_value(
			$assoc_args,
			'body',
			sprintf(
	...
	public function queue_list( $args, $assoc_args ) {
		...
		$items = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT id, to_email, subject, status, attempts, provider_used, scheduled_at, sent_at, error_message FROM {$table} {$where} ORDER BY id DESC LIMIT %d",
				...$params
			),
			ARRAY_A
		);
	...
	public function queue_flush( $args, $assoc_args ) {
		...
		// phpcs:enable WordPress.DB.DirectDatabaseQuery, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
	...
	public function log( $args, $assoc_args ) {
		...
		$items = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT id, level, message, provider, created_at FROM {$table} {$where} ORDER BY id DESC LIMIT %d",
				...$params
			),
			ARRAY_A
		);
	...
	public function provider( $args, $assoc_args ) {
		...
		$rows = array(
			...
			array( 'Backup Provider', $backup ? $backup : 'None' ),
	...
	public function subscriber_list( $args, $assoc_args ) {
		...
		$items = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT id, email, name, status, frequency, confirmed_at, created_at FROM {$table} {$where} ORDER BY id DESC LIMIT %d",
				...$params
			),
			ARRAY_A
		);
